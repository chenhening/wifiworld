...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	public void expandGroup(int index){
		if(mRefreshableView instanceof ExpandableListView){
			((ExpandableListView) mRefreshableView).expandGroup(index);
		}
	}

	 public void collapseGroup(int groupPos) {
		 if(mRefreshableView instanceof ExpandableListView){
				((ExpandableListView) mRefreshableView).collapseGroup(groupPos);
		}
	 }
		...
		if (null == adapter || adapter.isEmpty()) {
			...
			if (lastVisiblePosition >= lastItemPosition - 1) {
				...
				if (lastVisibleChild != null) {
					return lastVisibleChild.getBottom()-1 <= mRefreshableView.getBottom();
				}
			}
		}
	...
	protected boolean needResetToHeadBegin() {
		int count = mRefreshableView==null?0:mRefreshableView.getAdapter().getCount();
