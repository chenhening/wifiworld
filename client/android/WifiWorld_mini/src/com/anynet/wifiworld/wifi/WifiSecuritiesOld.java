...
public class WifiSecuritiesOld extends WifiSecurities {
    ...
    private static final String TAG = "WifiSecuritiesOld";
        ...
        if (security.equals(WEP)) {
            ...
            if (!TextUtils.isEmpty(password)) {
				if (wepPasswordType == WEP_PASSWORD_AUTO) {
                    if (isHexWepKey(password)) {
                        ...
                        config.wepKeys[0] = WifiAdmin.convertToQuotedString(password);
                    }
                } else {
                    config.wepKeys[0] = wepPasswordType == WEP_PASSWORD_ASCII
                            ? WifiAdmin.convertToQuotedString(password)
                            : password;
                }
            }
            ...
            if (!TextUtils.isEmpty(password)) {
                if (password.length() == 64 && isHex(password)) {
                    ...
                    config.preSharedKey = WifiAdmin.convertToQuotedString(password);
                }
            }
