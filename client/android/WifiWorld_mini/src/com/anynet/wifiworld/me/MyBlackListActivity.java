...
public class MyBlackListActivity extends BaseActivity {
	private final static String TAG = MyBlackListActivity.class.getSimpleName();
	
	//IPC
	private Context mContext;
	public static List<WifiReport> mListData;
	...
	private void bingdingTitleUI() {
		...
		mTitlebar.tvTitle.setText("我的举报");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tvtile.setText("当前举报的网络列表(向左滑动可删除)");
		
		//查询服务器
		WifiReport records = new WifiReport();
		records.Userid = LoginHelper.getInstance(this).getCurLoginUserInfo().getUsername();
		records.QueryWifiByUser(mContext, records.Userid, new MultiDataCallback<WifiReport>() {
            ...
            public boolean onSuccess(List<WifiReport> objects) {
				...
				for (WifiReport report : objects) {
					macs.add(report.MacAddr);
				}
				if (macs.size() > 0) {
	...
	private void displayList(final List<WifiProfile> wifiProfiles) {
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position,
					long id) {
				WifiReport wifiReport = mListData.get(position);
				WifiProfile wifiProfile = getWifiProfile(wifiReport.MacAddr, wifiProfiles);
				if (wifiProfile != null) {
	...
	class ListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			WifiReport wifiReport = mListData.get(position);
			WifiProfile wifiProfile = getWifiProfile(wifiReport.MacAddr, mWifiProfiles);
